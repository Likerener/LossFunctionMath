...
A \emph{set} is a collection of distinct, well-defined objects considered as a
whole. The objects are its \emph{elements}; we write $a \in A$ when $a$ is an
element of $A$. Two sets are equal iff they have the same elements (axiom of
extensionality):
\(
  A = B \iff (\forall x)(x \in A \leftrightarrow x \in B).
\)
A set may be specified by roster $\{a_1, \dots, a_n\}$ or by set-builder
notation $\{x \in U : P(x)\}$.
\end{definition}

\paragraph{Cartesian products.}

\begin{definition}[Cartesian product and power]
For sets $A, B$, the \emph{Cartesian product} is
\(
  A \times B := \{(a, b) : a \in A,\ b \in B\}.
\)
For finite sets $|A \times B| = |A| \cdot |B|$. The product of $n$ sets is
$A_1 \times \cdots \times A_n := \{(a_1, \dots, a_n) : a_i \in A_i\}$, and the
$n$-th Cartesian power of a single set is
...
$A \subseteq B$ iff every element of $A$ belongs to $B$. If additionally
$A \neq B$, $A$ is a \emph{proper subset} of $B$, written $A \subsetneq B$.
\end{definition}
